...
class InternationalJourneyTest extends FlatSpec with ShouldMatchers {
  ...
  trait WithInternationalJourney {
    ...
    val internationalJourney = InternationalJourney(timestamp, 966000000000L, Outbound, 966, 971)
  }
  ...
  it should "be built from CSV with Inbound scenario" in new WithInternationalJourney {
    fromCsv.fromFields(fields.updated(5, "1")) should be (internationalJourney.copy(userType = Inbound))
  }
}
