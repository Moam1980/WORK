...
class SubscriberProfilingViewTest extends FlatSpec with ShouldMatchers {
    ...
    val subscriberProfilingViewRoamer =
      SubscriberProfilingView(
        ...
        category = ProfilingCategory(
          ageGroup = EdmCoreUtils.UnknownKeyword,
          genderGroup = EdmCoreUtils.UnknownKeyword,
          nationalityGroup = ProfilingCategory.NationalityRoamers,
          affluenceGroup = EdmCoreUtils.UnknownKeyword))
  }
  ...
  trait WithSubscriberProfilingView {
    ...
    val row = Row("420030100040377", Row("16-25", "M", "Saudi Arabia", "Top 20%"))
    val wrongRow = Row("420030100040377", Row("16-25", 1, "Saudi Arabia", "Top 20%"))
  ...
  trait WithTotalRevenue extends WithSubscriber {
    ...
    val maxTotalRevenue = ProfilingCategory.MaxTotalRevenue
  ...
  it should "field and header have same size" in new WithSubscriberProfilingView {
    subscriberProfilingView.fields.length should be (SubscriberProfilingView.Header.length)
  }
  ...
  it should "return 16-25 as age group when age between 16 and 25" in new WithAgeGroup {
    for (i <- 16 to 25)
      ProfilingCategory.ageGroup(subscriber.copy(age = Some(i.toFloat))) should be (ageGroup16To25)
  }

  it should "return 26-60 as age group when age between 26 and 60" in new WithAgeGroup {
    for (i <- 26 to 60)
      ProfilingCategory.ageGroup(subscriber.copy(age = Some(i.toFloat))) should be (ageGroup26To60)
  }

  it should "return 61- as age group when age greater than 60" in new WithAgeGroup {
    for (i <- 61 to 110)
      ProfilingCategory.ageGroup(subscriber.copy(age = Some(i.toFloat))) should be (ageGroupGreaterThan60)
  }

  it should "return Unknown as age group when age not defined" in new WithSubscriber {
    ProfilingCategory.ageGroup(subscriber.copy(age = None)) should be (EdmCoreUtils.UnknownKeyword)
  }

  it should "return Unknown as age group when age less than 16" in new WithSubscriber {
    for (i <- 0 to 15)
      ProfilingCategory.ageGroup(subscriber.copy(age = Some(i.toFloat))) should be (EdmCoreUtils.UnknownKeyword)
  }

  it should "return correct male gender group" in new WithSubscriber {
    ProfilingCategory.genderGroup(subscriber.copy(gender = Male)) should be ("M")
  }

  it should "return correct female gender group" in new WithSubscriber {
    ProfilingCategory.genderGroup(subscriber.copy(gender = Female)) should be ("F")
  }

  it should "return unkwown gender group" in new WithSubscriber {
    ProfilingCategory.genderGroup(subscriber.copy(gender = UnknownGender)) should be (EdmCoreUtils.UnknownKeyword)
  }

  it should "return Saudi Arabia when nationality inferred is Saudi Arabia" in new WithNationalities {
    ProfilingCategory.nationalityGroup(
      subscriber.copy(
  ...
  it should "return roamers when nationality inferred is not Saudi Arabia and MCC is not Saudi Arabia" in
    new WithNationalities {
      ProfilingCategory.nationalityGroup(
        subscriber.copy(
  ...
  it should "return non saudi when nationality inferred is not Saudi Arabia and MCC is not Saudi Arabia" in
    new WithNationalities {
      ProfilingCategory.nationalityGroup(
        subscriber.copy(
          nationalities = nationalitiesOther,
  ...
  it should "return total revenue when total revenue and subscriber neither large corporate not post paid" in
    new WithTotalRevenue {
      ProfilingCategory.totalRevenue(subscriber) should be (subscriber.revenues.totalRevenue)
    }

  it should "return age groups" in new WithSubscriber {
    ProfilingCategory.AgeGroups should be (Array("16-25", "26-60", "61-", EdmCoreUtils.UnknownKeyword))
  }

  it should "return gender groups" in new WithSubscriber {
    ProfilingCategory.GenderGroups should be (Array("M", "F", EdmCoreUtils.UnknownKeyword))
  }

  it should "return nationality groups" in new WithSubscriber {
    ProfilingCategory.NationalityGroups should be (
      Array("Saudi Arabia", "Roamers", "Non-Saudi", EdmCoreUtils.UnknownKeyword))
  }

  it should "return affluence groups" in new WithSubscriber {
    ProfilingCategory.affluenceGroups should be (
