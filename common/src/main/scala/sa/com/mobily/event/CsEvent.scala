...
/**
 * Because we are serializing with Avro and are implementing put method from SpecificRecord we need declare mutable
 * variables so it can be set their values at deserialize.
 */
case class CsEvent(
  var beginTime: Long,
  var ci: String,
  ...
  var rat: Short,
  var sac: String,
  var tac: String) extends SpecificRecordBase with SpecificRecord {
